...
function calculateRent() {
  ...
  const adminRate = adminType === 'new' ? s.adminNewGross : adminType === 'alvi' ? s.adminAlviGross : s.adminAlviGross;
  ...
  const parts = [
    ['Captador do imóvel / relocação', net * s.captor / 100, pct(s.captor), $('rentCaptor').value],
    ['Relocação administrativa', net * s.relocation / 100, pct(s.relocation), $('rentRelocation').value],
    ['Administração Nova', net * adminDistributionRate / 100, pct(adminDistributionRate), $('rentAdminResponsible').value],
  ...
  $('rentLines').innerHTML =
    ...
    line('Adm', -admin, adminType === 'none' ? 'Sem desconto' : pct(adminRate)) +
...
const rentDefs = [
  ...
  ['relocation','Relocação administrativa','Distribuição do líquido','%'],
  ['newAdmin','Administração Nova','Distribuição do líquido na administração nova','%'],
